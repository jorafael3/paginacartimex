...
function checkDuplicate($login_cedula) // check for duplicated cedulas on web_loginpag
{
	...
	if ($ctr > 0) {
		$cedula_duplicate = true;
	}
	if ($ctr == 0) {
		$cedula_duplicate = false;
	}
...
if (isset($_POST["user"]) && isset($_POST["cedula"])) {
	...
	if ($clean_s == "4") {
		...
		if ($ced_status == true && $result_duplicate == false) {
			...
			$status_message = "El usuario no existe en el Dobra.";
		}
		if ($result_duplicate == true) {
			$status_message = "Ese RUC o CEDULA ya ha sido creado previamente en la tabla de la web.";
		}
	}
}
...
// READ USERS LIST
$estado = 'PENDIENTE';
$statement = $pdo->prepare("SELECT * FROM web_regDistribuidor WHERE Estado=:estado");
$statement->bindParam(':estado', $estado, PDO::PARAM_STR);
$statement->execute();
?>
<div class="container-fluid">
	<div class="d-sm-flex justify-content-between align-items-center mb-4">
		<h3 class="text-dark mb-0">Registros de Distribuidor Online</h3>
	</div>
							...
							<div class="col">
								<div class="form-group">
									<label for="usuario_id">
										<strong>Usuario CEDULA o RUC</strong>
									</label>
									<input class="form-control" type="text" placeholder="CEDULA / RUC" name="cedula">
								</div>
								<label class="radio-inline"><input type="radio" name="user" value="1">Ingresado</label><br>
								<label class="radio-inline"><input type="radio" name="user" value="2">Rechazado</label><br>
